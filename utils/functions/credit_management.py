...
def get_sale_credit_detail(customer=None, sale_master=None):
    ...
    if customer and sale_master:
        bills = SaleMain.objects.filter(pay_type=2, customer=customer, id=sale_master) \
        ...
        bills = SaleMain.objects.filter(pay_type=2, customer=customer) \
            .order_by('created_date_ad') \
            .values('customer_id', 'sale_no') \
            .annotate(
            total_amount=F('grand_total'),
        ...
        bills = SaleMain.objects.filter(pay_type=2, id=sale_master) \
            .order_by('created_date_ad') \
            .values('customer_id', 'sale_no') \
            .annotate(
            total_amount=F('grand_total'),
